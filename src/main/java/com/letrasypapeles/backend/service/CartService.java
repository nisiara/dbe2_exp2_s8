...
public class CartService {
  ...
  public OrderResponse processCartAndGenerateReceipt(CartRequest cartRequest) {
    ...
    // 3b. Inicializamos el total que guardara el valor del producto * la cantidad
    
    // 3c. Creamos una lista vacia que contendrá cada CartItem que llega de cartRequest
...
private List<CartItem> processCartItems(List<CartItemRequest> items, Cart cart) {
  return items.stream().map(item -> {
    ...
    return CartItem.builder()
      ...
      .price(product.getPrice()) // Precio actual en BD
      .build();
